...
Using a stable document ID means retries and repeated exports for the same user and date update the existing document instead of creating duplicates. This also applies to today's in-progress data: the app can upload a partial row earlier in the day, then upload fresher values later, and the later payload replaces the same `apple-health-data::<user_id>::<date>` document. The API stores both `user_id` and `userID`, so the raw-data container can be partitioned by either field. Missing-date checks filter on `type = "apple-health-data"`, so other health sources can share the same container.
